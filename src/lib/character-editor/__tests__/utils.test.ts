...
describe("charactersEqual", () => {
  it("returns true for identical characters", () => {
    ...
    expect(charactersEqual(charA, charB)).toBe(true);
  });

  it("returns true for empty characters of same size", () => {
    const charA = {
      pixels: [
        [false, false],
        [false, false],
  ...
  it("returns true for all-filled characters of same size", () => {
    const charA = {
      pixels: [
        [true, true],
        [true, true],
  ...
  it("returns false when pixel values differ", () => {
    const charA = {
      pixels: [
        [true, false],
        [false, true],
    ...
    expect(charactersEqual(charA, charB)).toBe(false);
  });
  ...
  it("returns true for empty pixel arrays", () => {
    const charA = { pixels: [] as boolean[][] };
    const charB = { pixels: [] as boolean[][] };
    expect(charactersEqual(charA, charB)).toBe(true);
  });

  it("returns false when one has empty pixels and other does not", () => {
    const charA = { pixels: [] as boolean[][] };
    const charB = {
      pixels: [[true]],
    };
  ...
  it("handles single pixel characters", () => {
    const charA = { pixels: [[true]] };
    const charB = { pixels: [[true]] };
    const charC = { pixels: [[false]] };

    expect(charactersEqual(charA, charB)).toBe(true);
    expect(charactersEqual(charA, charC)).toBe(false);
  });

  it("handles large characters", () => {
    const createLargeChar = (fill: boolean) => ({
      pixels: Array(16)
        .fill(null)
        .map(() => Array(16).fill(fill)),
    });

    const charA = createLargeChar(true);
    const charB = createLargeChar(true);
    const charC = createLargeChar(false);

    expect(charactersEqual(charA, charB)).toBe(true);
    expect(charactersEqual(charA, charC)).toBe(false);
  });

  it("detects difference in first pixel", () => {
    const charA = {
      pixels: [
        [true, true, true],
        [true, true, true],
  ...
  it("detects difference in last pixel", () => {
    const charA = {
      pixels: [
        [true, true, true],
        [true, true, true],
